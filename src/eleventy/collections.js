...
export const features = (collection) => {
	...
	const filteredCollection = collection.getFilteredByTag("feature").filter(isPublished).filter(notReply).sort(dateSort)
...
export const notesWithoutReplies = (collection) => {
	...
	const filteredCollection = collection.getFilteredByTag("note").filter(isPublished).filter(notReply).sort(dateSort)
...
export const onThisDay = (collection) => {
	...
	const filteredCollection = blogPosts(collection)
		...
		.filter((item) => {
			return new Date(item.data.date).getFullYear() !== currentYear && new Date(item.data.date).getDate() === currentDay && new Date(item.data.date).getMonth() === currentMonth
		})
