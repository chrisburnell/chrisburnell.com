...
export const latitude = 44.6463;
export const longitude = -63.6162;
export const description = `I’m <span class=" [ canada ] [ p-name ] ">${metadata.name}</span> <button id="name" class=" [ anchor  small ] [ js-name-button  no-feed ] " title="Hear my name read aloud" tabindex="-1">/<span title="'k' in 'kind'">k</span><span title="'r' in 'rye'">r</span><span title="'i' in 'rabbit'">ɪ</span><span title="'s' in 'sigh'">s</span> <span title="'b' in 'buy'">b</span><span title="/ɜːr/: 'ur' in 'fur'">ɜːr</span><span title="/ˈ/: primary stress follows">ˈ</span><span title="'n' in 'nigh'">n</span><span title="/ɛ/: 'e' in 'dress'">ɛ</span><span title="'l' in 'lie'">l</span>/</button> (${metadata.pronouns}), a <span class=" [ canada ] ">${nationality}</span> <span class=" [ p-job-title ] ">${role}</span> living in <a href="https://www.openstreetmap.org/#map=13/${latitude}/${longitude}" class=" [ p-locality ] ${metadata.location.includes("Canada") ? "[ canada ] " : ""}">${metadata.location}</a>. I’m often found <a href="/projects/">rabbit hole deep-diving</a>, and this is where I think, write, and share my passion for front end development and the web.`;
export const gpg_styled = metadata.gpg.replace(
	/(.{4})/g,
