...
class Welcome {
	 ...
	 */
	constructor() {
		const serviceWorkerStatus = navigator?.serviceWorker?.controller?.state || "pending"
		...
		const divider = "-".repeat(Math.max(operatingSystemString.length, languageString.length, networkString.length, serviceWorkerString.length))
