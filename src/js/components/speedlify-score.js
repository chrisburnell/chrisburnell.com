class SpeedlifyUrlStore {
	...
	async fetchHash(speedlifyUrl, url) {
		...
		const apiUrl = SpeedlifyUrlStore.normalizeUrl(
			speedlifyUrl,
			"api/urls.json",
		);
...
`;
	...
	connectedCallback() {
		...
		this.speedlifyUrl = this.getAttribute(
			SpeedlifyScore.attrs.speedlifyUrl,
		);
		this.shorthash = this.getAttribute(SpeedlifyScore.attrs.hash);
		this.rawData = this.getAttribute(SpeedlifyScore.attrs.rawData);
		this.url =
			this.getAttribute(SpeedlifyScore.attrs.url) || window.location.href;

		if (!this.rawData && !this.speedlifyUrl) {
			console.error(
				`Missing \`${SpeedlifyScore.attrs.speedlifyUrl}\` attribute:`,
				this,
			);
			return;
		}
	...
	async init() {
		...
		if (!hash) {
			console.error(
				`<speedlify-score> could not find hash for URL (${this.url}):`,
				this,
			);
			return;
		}
	...
	render(data = {}) {
		...
		// no extra attributes
		if (
			(!this.hasAttribute(attrs.requests) &&
				!this.hasAttribute(attrs.weight) &&
				!this.hasAttribute(attrs.rank) &&
				!this.hasAttribute(attrs.rankChange)) ||
			this.hasAttribute(attrs.score)
		) {
			content.push(
				this.getScoreHtml("Performance", data.lighthouse?.performance),
			);
			content.push(
				this.getScoreHtml(
					"Accessibility",
		...
		if (this.hasAttribute(attrs.rank)) {
			...
			meta.push(
				`<${rankUrl ? `a href="${rankUrl}"` : "span"} class="rank">${data.ranks?.cumulative}</${rankUrl ? "a" : "span"}>`,
			);
		}
		if (this.hasAttribute(attrs.rankChange) && data.previousRanks) {
			...
			meta.push(
				`<span class="rank-change ${change > 0 ? "up" : change < 0 ? "down" : "same"}">${change !== 0 ? Math.abs(change) : ""}</span>`,
			);
		}
		if (meta.length) {
