...
class ClampCalculator {
	...
	calculate() {
		...
		const preferredSizePixels = this.maxDecimals(
			sizeMax - viewportMax * change,
		);
		const preferredSize = this.toRem(
			sizeMax - viewportMax * change,
			fontSizeRoot,
		);
		...
		this.visualRoot.querySelector("span").innerText =
			`Root Size: ${fontSizeRoot}px`;
		this.visualOutput.querySelector("span").innerText =
			`Clamped Size: ${sizeMin}–${sizeMax}px`;
...
Maximum Size ≈ ${this.maxDecimals(sizeMax - viewportMax * change + viewportMax * change)}px`;
	 ...
	 */
	measureViewport() {
		this.visualCalculated.querySelector("span").innerText =
			`Current Size: ${this.maxDecimals(this.visualOutput.querySelector(".size-example").getBoundingClientRect().width, 3)}px`;
		this.visualViewport.querySelector("span").innerText =
			`Viewport Size: ${window.innerWidth}px × ${window.innerHeight}px`;
	}
