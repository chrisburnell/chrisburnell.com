...
class InteractiveSparkline {
	...
	processSparkline(inputs) {
		...
		inputs.inputValues.value = inputs.inputValues.value.replace(
			/(?![0-9])./gim,
			"",
		);
		inputs.inputLineWidth.value = inputs.inputLineWidth.value.replace(
			/(?![0-9])./gim,
			"",
		);

		inputs.component.setAttribute(
			"values",
			inputs.inputValues.value.split(""),
		);
		inputs.component.setAttribute(
			"line-width",
			inputs.inputLineWidth.value,
		);
		...
		inputs.component.setAttribute(
			"endpoint-color",
			inputs.inputEndpointColor.value,
		);
	}
}
