	...
	for (const keys of [keysCorePages, keysPages]) {
		for (const request of keys) {
			if (!request.url.includes("/offline/")) {
				const response =
					(await cacheCorePages.match(request)) ||
					(await cachePages.match(request));
				...
				if (dom.querySelector(".description .dt-published")) {
					data.published = new Date(
						dom
							.querySelector(".description .dt-published")
							.getAttribute("datetime"),
					).toISOString();
					data.publishedString = dom.querySelector(
						".description .dt-published",
					).innerText;
				}
